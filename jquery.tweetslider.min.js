(function($){var defaults={username:'',hash:'Twitter',slides:5,width:500,speed:'fast',refreshTime:600000};var storage={store:{},init:function(element){storage.element=element;if(!storage.element.hasClass('tweet_slider'))
...
{options.params="&include_entities=1";if(options.username!=''&&options.username.length>0)
{console.log('going into user grabber');options.api='https://api.twitter.com/1/statuses/user_timeline.json';options.params+="&screen_name="+options.username+"&count="+options.slides;}
else{console.log('going into search grabber');options.api='http://search.twitter.com/search.json';options.params+="&q="+options.hash+"&result_type=recent&rpp="+options.slides+"&page=1";}
$.getJSON(options.api+"?callback=?",options.params,function(data)
...
for(l in hashtags)
{text=text.replace("#"+hashtags[l].text,"<a style='underline:none' href='http://twitter.com/#!/search/#"+hashtags[l].text+"'>#"+hashtags[l].text+"</a>");}
for(l in user_mentions)
{text=text.replace("@"+user_mentions[l].screen_name,"<a style='underline:none' href='http://twitter.com/#!/"+user_mentions[l].screen_name+"'>@"+user_mentions[l].screen_name+"</a>");}
