(function($){var defaults={username:'Twitter',slides:5,baseURL:'https://api.twitter.com/1/statuses/user_timeline.json',width:500,speed:'fast',refreshTime:600000};var storage={store:{},init:function(element){storage.element=element;if(!storage.element.hasClass('tweet_slider'))
{storage.element.addClass('tweet_slider')}
storage.element.data('currslide',new Number(1));$('<h4></h4>').html('We Tweet').prependTo(storage.element);storage.tweets_container=$('<ul></ul>');storage.tweets_container.appendTo(storage.element);},grab_tweets:function(options)
{$.getJSON(options.baseURL+"?callback=?","&screen_name="+options.username+"&count="+options.slides+"&include_entities=1",function(data)
{storage.store.tweets=data;storage.build_display();storage.addButtons(1);storage.element.css('width',options.width+'px');storage.csswidth=data.length*options.width;storage.twidth=storage.csswidth-options.width;storage.tweets_container.css('width',storage.csswidth+'px');});setTimeout(storage.refresh,options.refreshTime);},build_display:function(){if(typeof(storage.slides)!="undefined"&&storage.slides.length>0)
{storage.tweets_container.html('').fadeIn('fast');}
var tweets=storage.store.tweets;var exp=/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;for(i in tweets)
{var tweet_html=$('<li></li>');tweet_html.css('width',options.width+'px').append('<div><img align="left" src="'+tweets[i].user.profile_image_url+'" />');tweet_html.append('<span><a href="http://www.twitter.com/'
+
tweets[i].user.screen_name+'">'+tweets[i].user.screen_name
+'</a>&nbsp;'+tweets[i].text.replace(exp,"<a href='$1'>$1</a>")+'</span>').append('</div>').appendTo(storage.tweets_container);}
if($(".controls").length<1)
{storage.controls=$('<div class="controls"></div>').hide('fast');storage.prevButton=$('<a href="#" class="prevButton"></a>');storage.prevButton.bind('click.tweetslider',function(e){e.preventDefault();clearInterval(storage.timer);storage.timer=setInterval(storage.slide,10000);storage.slide("prevSlide");});storage.nextButton=$('<a href="#" class="nextButton"></a>');storage.nextButton.bind('click.tweetslider',function(e){e.preventDefault();clearInterval(storage.timer);storage.timer=setInterval(storage.slide,10000);storage.slide("nextSlide");});storage.nextButton.hide().appendTo(storage.controls);storage.prevButton.hide().appendTo(storage.controls);storage.controls.show('fast').appendTo(storage.element);}
